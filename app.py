...
st.markdown("""
    ...
    .hex-image { width: 80px; height: 80px; object-fit: contain; margin-bottom: 10px; opacity: 0.8; }
    ...
    /* Attached Info Box */
    .attached-info-box { 
        background-color: #ffffff; width: 210px; padding: 20px 15px 15px 15px; 
        border-radius: 0 0 10px 10px; border: 2px solid #2d3436; 
        margin-top: -30px; z-index: 1; box-shadow: 0 4px 15px rgba(0,0,0,0.1); 
    }
    .info-list { font-size: 13px; color: #2d3436; line-height: 1.8; border-bottom: 1px solid #dfe6e9; padding: 4px 0; }
    .stButton>button { width: 100%; border-radius: 5px; border: 1px solid #2d3436; background-color: #ffffff; color: #2d3436; font-weight: 700; }
...
def generate_momu_sketch_logo():
    ...
    ax.text(6, -0.6, "The Integrated Molecular Analyzing Pipeline", color='#636e72', fontsize=14)
    return fig
...
with tabs[0]:
    ...
    with c1:
        st.markdown(f"""<div class="benzene-cluster"><div class="diamond-hexagon">
            ...
            <p class="hexagon-subtitle">Analysis</p><p class="click-here-accent">info</p>
        </div></div>""", unsafe_allow_html=True)
        if st.button("Toggle Protein Info", key="btn1"):
        ...
        if st.session_state.show_desc["p1"]:
            ...
            for i in ["Extract Sequence", "Molecular Weight", "pI Calculation", "Instability Index"]:
    ...
    with c2:
        st.markdown(f"""<div class="benzene-cluster"><div class="diamond-hexagon">
            ...
            <p class="hexagon-subtitle">Site Prediction</p><p class="click-here-accent">info</p>
        </div></div>""", unsafe_allow_html=True)
        if st.button("Toggle Site Info", key="btn2"):
            st.session_state.show_desc["p2"] = not st.session_state.show_desc["p2"]
        if st.session_state.show_desc["p2"]:
            st.markdown('<div class="benzene-cluster"><div class="attached-info-box">', unsafe_allow_html=True)
            for i in ["SASA Surface", "Binding Pockets", "Ligand Docking"]:
                st.markdown(f'<div class="info-list">~ {i}</div>', unsafe_allow_html=True)
            st.markdown('</div></div>', unsafe_allow_html=True)
    ...
    with c3:
        st.markdown(f"""<div class="benzene-cluster"><div class="diamond-hexagon">
            <p class="hexagon-title">Mutate</p>
            <img src="https://cdn.rcsb.org/images/structures/1/1aie/1aie_assembly-1.jpeg" class="hex-image">
            <p class="hexagon-subtitle">Prediction</p><p class="click-here-accent">info</p>
        </div></div>""", unsafe_allow_html=True)
        if st.button("Toggle Mutation Info", key="btn3"):
            st.session_state.show_desc["p3"] = not st.session_state.show_desc["p3"]
        if st.session_state.show_desc["p3"]:
            st.markdown('<div class="benzene-cluster"><div class="attached-info-box">', unsafe_allow_html=True)
            for i in ["B-Factor Analysis", "RMSF Calculation", "Energy Stability"]:
                st.markdown(f'<div class="info-list">~ {i}</div>', unsafe_allow_html=True)
            st.markdown('</div></div>', unsafe_allow_html=True)
